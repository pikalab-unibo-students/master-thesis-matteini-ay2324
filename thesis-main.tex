...
\acresetall{AI}
\sidenote{troppo lungo? forse meglio spezzare in più paragrafi}
...
There are also many differences between the socio-legal and technical perspectives. Regarding the \ac{AI} lifecycle, engineers tend to focus on technical aspects and few development phases, in fact the major part of the literature speaks only about \textit{pre-processing}, \textit{in-processing} and \textit{post-processing} (\Cref{fig:ai-lifecycle} \cite{DBLP:conf/ijcai/CalegariCMO23}).
\sidenote{la citazione va bene anche a fianco dell'immagine?}
%
\begin{figure}
    \centering
...
% Fairness notions vary by context and stakeholders, requiring different activities for fulfillment.
...
% 1. provide a formal description of the decision-based graph
